...
set TARGET_PROCESS=
for /f "usebackq tokens=1" %%a in (`echov cmd`)     do if /i "%%a" equ "%~1" set TARGET_PROCESS=cmd.exe -ArgumentList '/t:5F /k """cd /d %CD%"""'
for /f "usebackq tokens=1" %%a in (`echov ps5 wps`) do if /i "%%a" equ "%~1" set TARGET_PROCESS=powershell.exe -ArgumentList '-NoExit -Command cd """%CD%"""'
for /f "usebackq tokens=1" %%a in (`echov ps7 ps`)  do if /i "%%a" equ "%~1" set TARGET_PROCESS=pwsh.exe -ArgumentList '-NoExit -Command cd """%CD%"""'

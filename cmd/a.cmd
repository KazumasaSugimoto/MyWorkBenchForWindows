...
call ps.cmd -Command "Get-Item %TARGET% -Force | Select-Object Name, @{n='LastWriteTime';e={$_.LastWriteTime.ToISOLike()}}, Length, Mode, @{n='Hash';e={$_.GetFileHash().GetCaptionWithBase64() -replace ' \(', """`n("""}} | Format-Table -Wrap"
...
git log --follow -5 --pretty=format:"%%C(yellow)%%h %%C(reset)%%s %%C(yellow)%%cr %%C(cyan)%%ar %%C(auto)%%d%%C(reset)" -- %TARGET% 2>nul
exit /b 0
